...
router.post("/signUp", async (req, res, next) => {
  ...
  if (password != passwordCheck) {
    res.send(
      "<script>alert('비밀번호와 비밀번호 확인이 일치하지 않습니다.');window.location.href='/admin/signUp'</script>"
    );
  }
  if (adminCode != adminCodeNow) {
    res.send(
      "<script>alert('Admin code가 일치하지 않습니다.');window.location.href='/admin/signUp'</script>"
    );
  }
  const queryData = {
  ...
  if (queryResult) {
    res.send(
      "<script>alert('이미 존재하는 이메일입니다.');window.location.href='/admin/signUp'</script>"
    );
  }
  ...
  if (hashedPassword) {
    ...
    res.send(
      "<script>alert('성공적으로 가입되었습니다.');window.location.href='/admin/login'</script>"
    );
  }
});
  ...
  if (!queryResult) {
    ...
    if (email == queryEmail) {
      if (bcrypt.compareSync(password, queryPassword)) {
        if (isSaveID) {
          res.cookie("savedID", email, {
            //쿠키 savedId에 id 저장
            expires: new Date(Date.now() + 1000 * 60 * 60 * 24 * 90), //90일 후 만료
            httpOnly: true,
          });
          res.cookie("savedCheck", true, {
            //쿠키 saveCheck에 check 상태 저장
